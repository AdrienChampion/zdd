...
//! ZDD factory.
//!
//! Everything is hashconsed, so in the end all operations are performed through a factory. It
//! contains the hash cons table and the cash for the most common operations.
//!
...
/// Internal ZDD factory trait, creates a node without checking the consistency of the label.
trait ZddFactory<Label: Eq + Hash + Clone> {
    /// Creates a node following the ZDD rules, but without checking that the label is actually
    /// consistent with the kids.
...
pub trait ZddMaker<Label: Eq + Hash + Clone, Lft, Rgt> {
    ...
    /// ```
    ///
    /// So, if `lbl` is above all the labels in `lft` and `rgt` it corresponds to creating the node
    /// `Node(lbl, lft, rgt)` (modulo zdd creation rules regarding `HasOne` and `Zero`) with a
    /// slight overhead.
    ///
    /// If `lbl` is not above all the labels in `lft` and `rgt` however (as is the case in the
    ...
    ///
    /// Providing the actual node creation function is way too dangerous as there's a lot of room
    /// for screwing up the ZDD well-formed-ness.
